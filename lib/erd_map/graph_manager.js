class GraphManager {
  constructor({
    ...
    this.cbObj = cbObj
    this.nodesWithChunkIndex = this.#mapNodesToChunks()
  }
  ...
  #mapNodesToChunks() {
    ...
    this.chunkedNodes.forEach((chunk, i) => {
      chunk.forEach((node) => {
        nodesWithChunkIndex[node] = i
      })
    })
    return nodesWithChunkIndex
  }
  ...
  #revertTapSelection() {
    ...
    this.#setShiftX(this.#mouseX - this.#selectedLayout[this.windowObj.selectingNode][0])
    this.#setShiftY(this.#mouseY - this.#selectedLayout[this.windowObj.selectingNode][1])

    this.#nodesIndex.forEach((nodeName, i) => {
      const chunkIndex = this.nodesWithChunkIndex[nodeName]
      this.nodeSource.data["alpha"][i] = chunkIndex <= this.#displayChunksCount ? VISIBLE : TRANSLUCENT
    })
    this.#sourceEdges.forEach((source, i) => {
      const target = this.#targetEdges[i]
      const sourceIndex = this.nodesWithChunkIndex[source]
      const targetIndex = this.nodesWithChunkIndex[target]
      const displayEdge = sourceIndex <= this.#displayChunksCount && targetIndex <= this.#displayChunksCount && this.#selectedLayout[source] !== undefined && this.#selectedLayout[target] !== undefined
  ...
  get #selectedLayout() { return this.layoutsByChunk[this.#displayChunksCount] }
  // @param {Integer} value
