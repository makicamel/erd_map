class GraphManager {
  ...
  toggleHovered() {
    ...
    if (closestNodeName && minmumDistance < 0.005) {
      ...
      this.#nodesIndex.forEach((nodeName, i) => {
        ...
        this.nodeSource.data["radius"][i] = nodeName === closestNodeName ? EMPTHASIS_SIZE : BASIC_SIZE
        this.nodeSource.data["text_color"][i] = isConnectedNode ? HIGHLIGHT_TEXT_COLOR : BASIC_TEXT_COLOR
        this.nodeSource.data["text_outline_color"][i] = isConnectedNode ? "gray" : null
      })
      ...
      this.edgeSource.data["line_color"] = this.#sourceNodes.map((start, i) => {
        return [start, this.#targetNodes[i]].includes(closestNodeName) ? HIGHLIGHT_EDGE_COLOR : "gray"
      })
    } else {
      ...
      this.nodeSource.data["text_color"] = this.#nodesIndex.map(() => BASIC_TEXT_COLOR)
  ...
  #revertTapSelection() {
    const [nodesX, nodesY] = this.#updateNodeXY(this.#selectedLayout, this.windowObj.selectedNode)
  ...
  #applyTapSelection(tappedNode) {
    ...
    this.#updateNodeXY(this.#wholeLayout, tappedNode)
    this.#nodesIndex.forEach(nodeName => {
      this.#wholeLayout[nodeName][0] += this.#shiftX
      this.#wholeLayout[nodeName][1] += this.#shiftY
    })

    this.nodeSource.change.emit()
    this.edgeSource.change.emit()

    this.layoutProvider.graph_layout = this.#wholeLayout
    this.layoutProvider.change.emit()
  }
  ...
  #handleZoom() {
    ...
    const { closestNodeName } = this.#findClosestNodeWithMinmumDistance(this.layoutsByChunk[previousDisplayChunksCount], showingNodes)
    const [nodesX, nodesY] = this.#updateNodeXY(this.#selectedLayout, closestNodeName)
