class GraphManager {
  ...
  toggleHovered() {
    ...
    if (closestNodeName && minmumDistance < 0.005) {
      ...
      if (originalRadius) { this.nodeSource.data["radius"] = originalRadius }
  ...
  toggleDisplayTitleMode() {
    ...
    if (this.#showTitleMode) {
      graphRenderer.node_renderer.data_source = circleNodeDataSource
      this.displayTitleModeToggle.label = "Display mode: title"
    } else {
      graphRenderer.node_renderer.data_source = rectNodeDataSource
