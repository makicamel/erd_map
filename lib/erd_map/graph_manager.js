class GraphManager {
  ...
  toggleDisplayTitleMode() {
    ...
    if (this.#showTitleMode) {
      ...
      this.nodeSource.data["fill_color"] = this.nodeSource.data["circle_original_color"]
      // Workaround to force re-render
      plot.renderers = plot.renderers.filter(r => r !== this.rectRenderer)
    } else {
      this.displayTitleModeToggle.label = "Display mode: foreign key"
      this.nodeSource.data["fill_color"] = this.nodeSource.data["rect_original_color"]
      // Workaround to force re-render
      plot.renderers = plot.renderers.filter(r => r !== this.circleRenderer)
    }
    if (!plot.renderers.includes(this.circleRenderer)) { plot.renderers.push(this.circleRenderer) }
    if (!plot.renderers.includes(this.rectRenderer)) { plot.renderers.push(this.rectRenderer) }

    const { selectedLayout } = this.#getShowingNodesAndSelectedLayout()
    this.#updateLayout(selectedLayout)

    this.nodeSource.change.emit()
    this.graphRenderer.change.emit()
