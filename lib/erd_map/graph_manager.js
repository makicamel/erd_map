class GraphManager {
  ...
  #applyTap(tappedNode, { nodeTapped }) {
    if (this.#showingAssociation) {
      ...
      this.nodeSource.data["alpha"] = this.nodeSource.data["index"].map((nodeName) =>
        showingNodes.includes(nodeName) ? VISIBLE : TRANSLUCENT
      )
      this.edgeSource.data["alpha"] = this.edgeSource.data["start"].map((source, i) =>
  ...
  #setSelectingNodeLabel(value) {
    ...
    this.selectingNodeLabel.text = value ? `Showing: ${value}'s ${mode}` : ""
  }
  // @param {Boolean} value
