class GraphManager {
  ...
  toggleZoomMode() {
    ...
    if (this.#fixingZoom) {
      toggleZoomButton.label = "Fixing Zoom"
      toggleZoomButton.button_type = "danger"
    } else {
      toggleZoomButton.label = "Zooming"
      toggleZoomButton.button_type = "warning"
  ...
  #executeZoom({ previousDisplayChunksCount }) {
    ...
    this.nodeSource.data["alpha"] = this.#nodesIndex.map((nodeName) => {
      return this.#selectedLayout[nodeName] ? VISIBLE : TRANSLUCENT
    })
