class GraphManager {
  ...
  toggleHovered() {
    ...
    if (closestNodeName && minmumDistance < 0.005) {
      ...
      this.nodeSource.data["text_color"] = this.#nodesIndex.map((nodeName, i) => {
        const isConnectedNode = nodeName === closestNodeName || (selectedLayout[nodeName] && connectedNodes.includes(nodeName))
        return isConnectedNode ? HIGHLIGHT_TEXT_COLOR : BASIC_TEXT_COLOR
      })
      this.nodeSource.data["text_outline_color"] = this.#nodesIndex.map((nodeName, i) => {
        const isConnectedNode = nodeName === closestNodeName || (selectedLayout[nodeName] && connectedNodes.includes(nodeName))
        return isConnectedNode ? "gray" : null
      })
